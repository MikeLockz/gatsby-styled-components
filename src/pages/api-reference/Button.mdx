...
import { Button, Loader, Box } from "rimble-ui";

# Button documentation

Use the Button component when you want users to perform an action like connecting their Ethereum account or starting a transaction. It contains a label with an optional icon before or after the label. It maps to a `<button type='button'>` element and extends [Box](/api-reference/Box).

*Want to understand best practices for using this component?* Read [Button design guidelines](/design-guidelines/Button)
...
Button's icon inherits all the props from [Icon](/api-reference/Icon), but also includes these:
...
`Button` uses all the style props from [Box](/api-reference/Box), but also includes these:
